from hearthbreaker.tags.base import Buff, Function, Condition, Event
...
class CHARACTER_STATUS:
    ...
    NO_SPELL_TARGET = 12
    STOLEN = 13
    ATTACK_EQUALS_HEALTH = 14


class PLAYER_STATUS:
    SPELL_DAMAGE = 1
    DOUBLE_DEATHRATTLE = 2
    DOUBLE_BATTLECRY = 3
    POWER_TARGETS_MINIONS = 4
    HEAL_AS_DAMAGE = 5
...
class SetTo(Buff):
    def __init__(self, status, amount, multiplier=1, conditions=None, until=None):
        super().__init__(status, conditions, until)
        self.amount = amount
        self.multiplier = multiplier

    def update(self, base, actor, target):
        if isinstance(self.amount, int):
            return self.amount
        else:
            return self.amount.do(actor, target)
...
class Add(Buff):
    def __init__(self, status, amount=1, multiplier=1, conditions=None, until=None):
        super().__init__(status, conditions, until)
        self.amount = amount
        self.multiplier = multiplier
    ...
    def update(self, base, actor, target):
        if isinstance(self.amount, int):
            ...
            return base + self.amount.do(actor, target) * self.multiplier
...
class Multiply(Buff):
    def __init__(self, status, amount=1, conditions=None, until=None):
        super().__init__(status, conditions, until)
        self.amount = amount
    ...
    def update(self, base, actor, target):
        if isinstance(self.amount, int):
            ...
            return base * self.amount.do(actor, target)
...
class Subtract(Buff):
    def __init__(self, status, amount=1, multiplier=1, minimum=0, conditions=None, until=None):
        super().__init__(status, conditions, until)
        self.amount = amount
        self.multiplier = multiplier
    ...
    def update(self, base, actor, target):
        if isinstance(self.amount, int):
            return max(base + self.amount, self.minimum)
        else:
            return max(base + self.amount.do(actor, target) * self.multiplier, self.minimum)
    ...
    def __from_json__(self, status, amount=1, minimum=0, conditions=None, until=None):
        self.status = status
        if isinstance(amount, dict):
            self.amount = Function.from_json(**amount)
        else:
...
class SetTrue(Buff):
    def __init__(self, status, conditions=None, until=None):
        super().__init__(status, conditions, until)
